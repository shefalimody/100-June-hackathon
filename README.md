...
*Your AI's honest mate — is it actually good, or does it just run?*

A tool that helps a non-technical builder decide **what good looks like** for their AI feature —
write a golden set + rubric, paste the AI's outputs, and get back the exact failures they were about
to ship, in plain English with what to fix. Built for 100x Cohort 7 — **Track B**.
...
- **Computer decides (fixed):** golden set, rubric, run records, grade↔case↔run links, the pass/fail
  tally, calibration accuracy, Row-Level Security.
- **AI decides (judgment):** reading a pasted output and deciding pass/fail vs the rubric.
- **Anti-rubber-stamp:** calibration cases carry a verdict you already know; we check the grader gets
  those right before trusting it on new cases.
